...
const buildNewHighlight = ({ articleId, userId, text, color, position, annotations }) => {
  // Trim title and treat empty strings as "not provided"
  const providedTitle = annotations?.title?.trim() || null;
...
const buildUpdatedHighlight = (highlight, body) => {
  ...
  if (newAnnotations !== undefined) {
    if (newAnnotations === null) {
      ...
      if (naTitle !== undefined) {
        // If title is provided but empty/whitespace, regenerate from text
        const trimmedTitle = typeof naTitle === 'string' ? naTitle.trim() : naTitle;
        updatedHighlight.annotations.title = trimmedTitle || generateTitle(highlight.text);
      }
