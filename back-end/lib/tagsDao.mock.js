...
const tagsDao = {
  ...
  async delete(id) {
    ...
    if (removed) {
      ...
      // CASCADE DELETE: Remove this tag from all articles that have it
      const articlesDao = getArticlesDao();
      const allArticles = articlesDao.getAllArticlesForInternalUse();
      const articlesToUpdate = allArticles.filter(article => 
        article.tags && Array.isArray(article.tags) && article.tags.includes(String(id))
      );

      // Update each article to remove the deleted tag
      for (const article of articlesToUpdate) {
        const updatedTags = article.tags.filter(tag => String(tag) !== String(id));
