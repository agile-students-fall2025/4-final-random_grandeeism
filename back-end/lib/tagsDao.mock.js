...
const tagsDao = {
  ...
  async delete(id) {
    ...
    if (removed) {
      ...
      // Check for both string and numeric versions of the ID
      const stringId = String(id);
      const numericId = Number(id);
      
      const articlesToUpdate = allArticles.filter(article => 
        article.tags && Array.isArray(article.tags) && (
          article.tags.includes(stringId) || 
          article.tags.includes(numericId) ||
          article.tags.includes(id)
        )
      );

      console.log(`Cascade delete: found ${articlesToUpdate.length} articles with tag ${id}`);

      // Update each article to remove the deleted tag
      for (const article of articlesToUpdate) {
        const updatedTags = article.tags.filter(tag => 
          String(tag) !== stringId && 
          Number(tag) !== numericId && 
          tag !== id
        );
        console.log(`Updating article ${article.id}: ${article.tags.length} -> ${updatedTags.length} tags`);
